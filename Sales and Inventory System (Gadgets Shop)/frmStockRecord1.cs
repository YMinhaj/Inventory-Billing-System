    ...
    public partial class frmStockRecord1 : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                String sql = "SELECT StockID,Config.ConfigID,ProductName,Features,Price,Quantity,Totalprice,Stockdate,size1,color,barcode from Stock,Config where Stock.ConfigID=Config.ConfigID order by ProductName";
        ...
        private void txtProductname_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                String sql = "SELECT StockID,Config.ConfigID,ProductName,Features,Price,Quantity,Totalprice,Stockdate,size1,color,barcode from Stock,Config where Stock.ConfigID=Config.ConfigID and productname like '" + txtProductname.Text + "%' order by ProductName";
                cmd = new OleDbCommand(sql, con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                String sql = "SELECT StockID,Config.ConfigID,ProductName,Features,Price,Quantity,Totalprice,Stockdate,size1,color,barcode from Stock,Config where Stock.ConfigID=Config.ConfigID and barcode like '" + textBox1.Text + "%' order by ProductName";
                cmd = new OleDbCommand(sql, con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
