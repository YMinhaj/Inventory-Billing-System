    ...
    public partial class frmConfigRecord1 : Form
    {
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                obj.textBox1.Text = dr.Cells[5].Value.ToString();
                obj.textBox2.Text = dr.Cells[6].Value.ToString();
                obj.textBox3.Text = dr.Cells[7].Value.ToString();
                obj.textBox4.Text = dr.Cells[8].Value.ToString();
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                cmd = new OleDbCommand("SELECT * from Config where barcode like '" + textBox1.Text + "%' order by Productname", con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
                {
