    ...
    public partial class frmStock : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                con.Open();
                cmd = new OleDbCommand("SELECT StockId as [Stock ID], (productName) as [Product Name],Features,sum(Quantity) as [Quantity],Price,sum(TotalPrice) as [Total Price],(size1) as [size],(color) as [color],(barcode) as [barcode] from Config,Stock where Config.ConfigID=Stock.ConfigID group by Stockid, productname,features,price,size1,color,barcode having sum(Quantity > 0)  order by Productname", con);
        ...
        private void txtProduct_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                cmd = new OleDbCommand("SELECT StockId as [Stock ID], (productName) as [Product Name],Features,sum(Quantity) as [Quantity],Price,sum(TotalPrice) as [Total Price],(size1) as [size],(color) as [color],(barcode) as [barcode] from Config,Stock where Config.ConfigID=Stock.ConfigID and productname like '" + txtProduct.Text + "%' group by Stockid, productname,features,price,size1,color,barcode having sum(quantity > 0)  order by Productname", con);
                OleDbDataAdapter myDA = new OleDbDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Stock");
                myDA.Fill(myDataSet, "Config");
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                cmd = new OleDbCommand("SELECT StockId as [Stock ID], (productName) as [Product Name],Features,sum(Quantity) as [Quantity],Price,sum(TotalPrice) as [Total Price],(size1) as [size],(color) as [color],(barcode) as [barcode] from Config,Stock where Config.ConfigID=Stock.ConfigID and barcode like '" + textBox1.Text + "%' group by Stockid, productname,features,price,size1,color,barcode having sum(quantity > 0)  order by Productname", con);
                OleDbDataAdapter myDA = new OleDbDataAdapter(cmd);
                DataSet myDataSet = new DataSet();
                myDA.Fill(myDataSet, "Stock");
                myDA.Fill(myDataSet, "Config");
