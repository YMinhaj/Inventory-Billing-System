    ...
    public partial class frmInvoice : Form
    {
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (textBox3.Text == "")
                {
                    MessageBox.Show("Please Insert Delivery Charges", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txtCustomerID.Focus();
                    return;
                }
                ...
                string cb = "insert Into Sales(InvoiceNo,InvoiceDate,CustomerID,SubTotal,VATPercentage,VATAmount,GrandTotal,TotalPayment,PaymentDue,Remarks,dcharges) VALUES ('" + txtInvoiceNo.Text + "',#" + dtpInvoiceDate.Text + "#,'" + txtCustomerID.Text + "'," + txtSubTotal.Text + "," + txtTaxPer.Text + "," + txtTaxAmt.Text + "," + txtTotal.Text + "," + txtTotalPayment.Text + "," + txtPaymentDue.Text + ",'" + txtRemarks.Text + "','"+textBox3.Text+"')";
     ...
     DataGridViewCellEventArgs e)
        ...
        public void GetData()
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                String sql = "SELECT StockID,Config.ConfigID,ProductName,Features,Price,sum(Quantity),barcode from Stock,Config where Stock.ConfigID=Config.ConfigID group by StockID,productname,Price,Features,Config.ConfigID,barcode having sum(quantity > 0) order by ProductName";
                cmd = new OleDbCommand(sql, con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
        ...
        private void textBox2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                con = new OleDbConnection(cs);
                con.Open();
                String sql = "SELECT StockID,Config.ConfigID,ProductName,Features,Price,sum(Quantity),Config.barcode from Stock,Config where Stock.ConfigID=Config.ConfigID and barcode like '" + textBox2.Text + "%' group by StockID,productname,Price,Features,Config.ConfigID,Config.barcode having sum(quantity > 0) order by ProductName";
                cmd = new OleDbCommand(sql, con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
        ...
        private void textBox3_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txtTotal.Text = (Convert.ToInt32(txtTotal.Text) + Convert.ToInt32(textBox3.Text)).ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            frmCustomers fmc = new frmCustomers();
            fmc.Show();
        }
    }
}